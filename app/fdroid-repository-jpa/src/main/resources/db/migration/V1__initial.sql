...
CREATE TABLE AntiFeature (
  id INT GENERATED BY DEFAULT AS IDENTITY(START WITH 100, INCREMENT BY 1),
  name VARCHAR(255) NOT NULL,
  CONSTRAINT pk_antiFeature PRIMARY KEY (id),
  CONSTRAINT ak_antiFeature UNIQUE (name)
);

CREATE TABLE AppAntiFeature (
  id INT GENERATED BY DEFAULT AS IDENTITY(START WITH 100, INCREMENT BY 1),
  appId INT NOT NULL,
  antiFeatureId INT NOT NULL,
  CONSTRAINT pk_appAf PRIMARY KEY (id),
  CONSTRAINT ak_appAf UNIQUE (appId,antiFeatureId),
  CONSTRAINT fk_appAf_app FOREIGN KEY (appId) REFERENCES App(id)  ON delete CASCADE,
  CONSTRAINT fk_appAf_af FOREIGN KEY (antiFeatureId) REFERENCES AntiFeature(id)  ON delete CASCADE
);
